// Generates the Linethesis litepaper -> public/linethesis-litepaper.pdf
...
const doc = new PDFDocument({
  size: "A4",
  margin: 56,
  info: { Title: "Linethesis - Litepaper", Author: "Onenept Studios" },
});
doc.pipe(createWriteStream("public/linethesis-litepaper.pdf"));
...
// Title block
doc.fillColor(INK).font("Helvetica-Bold").fontSize(28).text("Linethesis");
...
h1("1. Abstract");
p(
  "In-play betting markets move fast, and the operators who post prices lose money at one specific moment: when their line is stale. The consensus re-prices on new information (a goal, a red card, a surge of danger) and a book that has not caught up is lifted at the old number. Linethesis is a read-only agent that watches that gap. It benchmarks a watched price against TxLINE's de-margined (no-vig) consensus, classifies every reference-line move as a clean move to FOLLOW or an overreaction to FADE, warns before the pickoff, and grades every call against on-chain ground truth. It is not a bookmaker, a market-maker, or a managed-trading service; it is the neutral, provable benchmark that sits beside the book.",
);
...
doc
  ...
  .fontSize(8)
  .text(
    "Linethesis - Onenept Studios - built on the TxLINE / TxODDS World Cup data layer. This document is informational, not an offer of securities or a solicitation to gamble.",
    { align: "center" },
  );

doc.end();
console.log("wrote public/linethesis-litepaper.pdf");
