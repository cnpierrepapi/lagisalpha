...
function casesForMatch(m) {
  ...
  for (const { signal, edge, frameTs } of latest.values()) {
    ...
    cases.push({
      ...
      liquidity: signal.liquidity ?? null,     // edge #2: 'thin'|'thick'|null (book regime)
      driftRegime: signal.driftRegime ?? null, // edge #2: 'carry'|'revert'|null
      lateMatch: signal.lateMatch ?? false,    // edge #6: closing ~20min
      pickoffRisk: signal.pickoffRisk ?? null, // escalated by thin book / late-match
