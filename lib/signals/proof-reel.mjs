...
// A steam → follow is RIGHT if the move HELD — i.e. the line did NOT give back most of its
// drift back toward the pre-event baseline by the close. (Model correction: flagged moves
// held 89%; ~11% reverted ≥50%.) Grading "did it continue PAST the entry peak" (clvPositive)
// was wrong: a move that keeps 90% of a +24pp jump but ticks 2pp off the peak is a correct
// follow, not a fade. HELD ⇔ reverted fraction (of the drift) < this.
const STEAM_REVERT_MAX = 0.5;
...
function casesForMatch(m) {
  ...
  for (const { signal, edge, frameTs } of latest.values()) {
    ...
    if (signal.kind === "overreaction") {
      ...
      clvReturn = round(clv.clvReturn, 4);
      clvPositive = clv.clvRight === true;
      // HELD verdict: did the move keep most of its drift (not revert toward baseline) by the
      // close? Measured on the SAME close frame shown on the card, so badge ≡ the three quotes.
      if (baseP != null && drifted > 0) {
        const revertedFrac = (drifted - Math.abs(obj.prob - baseP)) / drifted; // <0 = continued further
        success = revertedFrac < STEAM_REVERT_MAX;
      } else {
        success = clvPositive; // no baseline → fall back to CLV vs entry
      }
    }
