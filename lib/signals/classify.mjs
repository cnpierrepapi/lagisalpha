...
//
// WHY these actions (from certified papers):
//   • steam         (Croxson & Reade 2014) — the market prices real news efficiently. A clean
//                    move is TRUE → FOLLOW / tighten toward the reference. A lagging book that
//                    follows late is exactly what a sharp picks off.
//   • overreaction  (Choi & Hui 2014; De Bondt–Thaler) — the line OVERSHOOTS a *surprising*
//                    goal and reverts within minutes → HOLD (don't chase) and, when confident,
//                    FADE. Our own data: 84% of these reverted in our favour (6-match pilot).
//   • pregoal_warning — the momentum tape (high_danger_possession / PossibleEvent.Goal) fires
//                    seconds BEFORE the line jumps → SUSPEND-SUGGESTED (your in-play line is
//                    about to go stale). Near-exclusive to TxLINE's granular feed.
//
...
export function classifyEdge(edge, ctx = {}) {
  ...
  // action: steam → follow (the move is real); overreaction → hold, escalate to fade when confident.
  const action = edge.kind === "steam" ? "follow" : confidence >= FADE_CONF ? "fade" : "hold";
  const revertLikely = edge.kind === "overreaction";
