...
export function classifyEdge(edge, ctx = {}) {
  ...
  const liqRegime = liquidityRegime(quoteDensity);
  const liquidity = liqRegime.liquidity; // 'thin'|'thick'|null — a neutral fact for any kind
  // carry/revert is a CONTINUATION call — coherent only for STEAM ("does this move carry, or
  // is it noise that snaps back?"). A goal-driven overreaction is decisive and tends to STICK
  // regardless of liquidity (Croxson-Reade; our steam-follow-89%), so edge #2's non-goal
  // base rate does NOT transfer to it. We leave driftRegime null there rather than print a
  // "revert" that reads as a (usually wrong) prediction next to a "held" verdict.
  const driftRegime = edge.kind === "steam" ? liqRegime.driftRegime : null;
  ...
  return {
    ...
    // regime/late annotations are steam-axis (continuation) claims → only annotate steam,
    // so an overreaction note never carries a "revert"/"drift amplifies" line that fights
    // its own verdict. Overreaction keeps its plain note; `liquidity` is still exposed as data.
    note:
      edge.kind === "steam" && (driftRegime || lateMatch)
