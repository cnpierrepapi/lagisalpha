...
export function calibrate(settled) {
  ...
  // edge #2 evidence: does the liquidity gate actually separate the calls? Bucket the
  // settled follow/hold record by the book regime it fired in — thick/'carry' (a real
  // move that should hold) vs thin/'revert' (noise that shouldn't). Only rows the engine
  // tagged (liquidity != null) count; null = older signals without a density proxy.
  const byLiquidity = Object.fromEntries(
    ["thick", "thin"].map((liq) => [liq, bucket(rows.filter((r) => r.liquidity === liq))]),
  );
