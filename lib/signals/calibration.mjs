...
function bucket(rows) {
  ...
  const n = done.length;
  // "right" uses the per-action verdict: follow/hold → held (FCV within the ±10pp band);
  // fade → CLV-positive (reversion). Falls back to clvRight when `correct` isn't present.
  const right = done.filter((r) => (r.correct != null ? r.correct : r.clvRight)).length;
...
export function calibrate(settled) {
  ...
  // edge #2 evidence: does the liquidity gate actually separate the calls? Carry-vs-revert
  // is a CONTINUATION claim, so we bucket STEAM follows only (thick should carry/hold, thin
  // should revert). Overreaction holds are goal-driven/decisive — out of the domain the
  // thick=carry/thin=revert base rate was estimated on, so they're excluded here.
  const byLiquidity = Object.fromEntries(
    ["thick", "thin"].map((liq) => [liq, bucket(rows.filter((r) => r.liquidity === liq && r.kind === "steam"))]),
  );
  ...
  return {
    ...
    byKind,
    byAction,
    byLiquidity, // edge #2: hit-rate split by book regime (thick=carry vs thin=revert)
    breadth: {
      matches: byFix.size,
    ...
    headline: (() => {
      const f = byAction.follow;
      const fa = byAction.fade;
      if (f && f.n > 0) {
        return `follow/hold held ${f.right}/${f.n} (FCV ±10pp, ${((f.hitRate ?? 0) * 100).toFixed(0)}%) · fade reverted ${fa?.right ?? 0}/${fa?.n ?? 0}, across ${byFix.size} matches`;
      }
      return `${byFix.size} matches settled`;
    })(),
  };
}
