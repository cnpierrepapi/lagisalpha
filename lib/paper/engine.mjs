...
/** Kelly fraction, clamped to [0,1]. gap and entry in the bought side's frame. */
...
export function openPosition(session, signal) {
  ...
  const f = Number.isFinite(signal.suggestedKellyF) ? signal.suggestedKellyF : kellyFraction(signal.fair, entry);
  const stake = CENTS(Math.min(availableCash(session), session.bankroll * f));
