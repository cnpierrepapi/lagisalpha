# Lagisalpha EC2 live desk worker
...
scp -i ~/Downloads/predmkt.pem -r ./lagisalpha ubuntu@63.33.24.157:~/lagisalpha

# on the box — Node is already installed via nvm; install just tsx
ssh -i ~/Downloads/predmkt.pem ubuntu@63.33.24.157
cd ~/lagisalpha/worker && npm install        # installs tsx only (no Next/React)
```

## 3. Env file

Create `~/lagisalpha/worker/.env` on the box (values from your local
